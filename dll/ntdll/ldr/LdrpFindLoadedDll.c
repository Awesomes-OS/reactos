                          ...
                          LDRP_LOAD_CONTEXT_FLAGS Flags)
{
    ...
    *LdrEntry = NULL;
    ...
    DPRINT("LDR: %s(\"%wZ\", %p, ..., %#X)\n", __FUNCTION__, DllName, PathSearchContext, Flags);

    if (Flags.BaseNameOnly)
    {
        Status = LdrpFindLoadedDllByName(DllName, NULL, Flags, LdrEntry, DdagState);
    }
    else if (Flags.HasFullPath)
    {
        Status = LdrpFindLoadedDllByName(NULL, DllName, Flags, LdrEntry, DdagState);
    }
    ...
    if (!Flags.BaseNameOnly && !NT_SUCCESS(Status))
    {
        ...
        {
            Status = LdrpFindLoadedDllByName(&BaseDllName, &FullDosDllPath, Flags, LdrEntry, DdagState);

            if (Status == STATUS_DLL_NOT_FOUND)
            {
                Status = LdrpFindLoadedDllByMappingFile(&NtName.String, LdrEntry, DdagState);
            }
        }
    ...
    if (DdagState)
        DPRINT("LDR: %s(\"%wZ\", %p, %p, %d, %#X) -> 0x%08lX\n",
               __FUNCTION__,
               DllName,
               PathSearchContext,
               *LdrEntry,
        ...
        DPRINT("LDR: %s(\"%wZ\", %p, %p, NULL, %#X) -> 0x%08lX\n",
               __FUNCTION__,
               DllName,
               PathSearchContext,
               *LdrEntry,
                  ...
                  PLDR_DATA_TABLE_ENTRY *LdrEntry)
{
    ...
    Status = LdrpFindLoadedDllInternal(&DllName.String, PathSearchContext, LdrEntry, &DdagState, NewContextFlags);

    if (!NT_SUCCESS(Status))
        goto Quickie;

    if (NT_SUCCESS(Status) && DdagState < LdrModulesCondensed)
    {
