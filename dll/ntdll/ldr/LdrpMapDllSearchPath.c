...
LdrpMapDllSearchPath(IN PLDRP_LOAD_CONTEXT LoadContext)
{
    ...
    PLDR_DATA_TABLE_ENTRY LdrEntry = NULL, LoadEntry = LoadContext->Entry;
    NTSTATUS Status;
    ...
    while (TRUE)
    {
        ...
        Status = LdrpFindExistingModule(&BaseDllName, &FullDosDllPath, LoadContext->Flags, BaseNameHashValue, &LdrEntry);
