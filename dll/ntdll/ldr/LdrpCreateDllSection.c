                     ...
                     OUT PHANDLE SectionHandle)
{
    ...
    if (!*FileHandle)
    {
        ...
        for (int i = 0; i <= 1; ++i)
        {
            ...
            if (ShowSnaps)
            {
                DPRINT1("LDR: %s - NtOpenFile failed; status = 0x%08lX\n",
                        __FUNCTION__,
                        Status);
            }
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        *SectionHandle = NULL;
        ...
        {
            DPRINT1("LDR: %s - NtCreateSection failed; status = 0x%08lX\n",
                    __FUNCTION__,
                    Status);
        }
        ...
        if (Status != STATUS_COMMITMENT_LIMIT && Status != STATUS_NO_MEMORY && Status != STATUS_INSUFFICIENT_RESOURCES)
        {
            ...
            /* Raise the error */
            if (NT_SUCCESS(ZwRaiseHardError(STATUS_INVALID_IMAGE_FORMAT,
                1,
                1,
                HardErrorParameters,
                OptionOk,
