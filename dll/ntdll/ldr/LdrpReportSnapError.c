                    ...
                    BOOLEAN Static)
{
    switch (Status)
    {
        ...
        {
            DPRINT1("Locating export at ordinal %lu failed with status: 0x%08lX. [BaseAddress=%p, ModuleName=%wZ]\n",
                    Ordinal,
                    Status,
                    BaseAddress,
                    ModuleName);
        ...
        {
            DPRINT1("Locating export \"%wZ\" failed with status: 0x%08lX. [BaseAddress=%p, ModuleName=%wZ]\n",
                    FunctionName,
                    Status,
                    BaseAddress,
                    ModuleName);
    ...
    if (Static)
    {
        ...
        if (NT_SUCCESS(ErrorStatus) && LdrpInLdrInit)
