...
{
    UNICODE_STRING SeparatorSource = RTL_CONSTANT_STRING(L"\\");
    NTSTATUS Status;
    ...
    {
        CurrentLazyEvaluationContext = LdrpAllocateHeap(HEAP_ZERO_MEMORY, sizeof(LDRP_LAZY_PATH_EVALUATION_CONTEXT));
    ...
    DPRINT("I fucked up and forgot to delete this debug code block.\n");
    ...
    _SEH2_TRY
    {
        ...
        for (;; ++CurrentIterator)
        {
            ...
            {
                RtlAppendUnicodeStringToString(&Buffer, &PathSource);
                WCHAR *LastPathChar = Buffer.Buffer + Buffer.Length - 1;
                if (*LastPathChar != L'\\' && *LastPathChar != L'/')
                    RtlAppendUnicodeStringToString(&Buffer, &SeparatorSource);
                RtlAppendUnicodeStringToString(&Buffer, DllName);
