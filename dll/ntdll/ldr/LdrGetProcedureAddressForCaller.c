                                ...
                                IN PVOID* CallbackAddress)
{
    ...
    if (FunctionNameInput)
    {
        ...
        {
            FunctionName = LdrpHeapAlloc(0, sizeof(ANSI_STRING));
            if (!FunctionName)
                return STATUS_INSUFFICIENT_RESOURCES;

            FunctionName->Buffer = LdrpHeapAlloc(0, FunctionNameInput->Length + 1);
    ...
    while (TRUE)
    {
        ...
        if (LdrEntryState >= MinimumRequiredState)
        {
            // Ok, found one, it is in appropriate state, use it.
            break;
        }
