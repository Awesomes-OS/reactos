                            ...
                            OUT PVOID* ProcedureAddress)
{
    if (Name)
    {
        ...
        const UINT32 OrdinalResult = LdrpNameToOrdinal(Name,
                                                       ...
                                                       BaseAddress,
                                                       PTR_ADD_OFFSET(BaseAddress, ExportDir->AddressOfNames),
                                                       PTR_ADD_OFFSET(BaseAddress, ExportDir->AddressOfNameOrdinals));
    ...
    const PULONG32 FunctionTable = PTR_ADD_OFFSET(BaseAddress, ExportDir->AddressOfFunctions);
                        ...
                        OUT PVOID *ProcedureAddress)
{
    ...
    if (!ExportDir)
    {
        DPRINT1("LDR: %s(%p, \"%s\", %lu, ...): No exports\n",
                __FUNCTION__,
                BaseAddress,
                Name,
                Ordinal);
