...
LdrpAllocateFileNameBufferIfNeeded(PLDRP_UNICODE_STRING_BUNDLE Destination, ULONG Length)
{
    ...
    {
        NewBuffer = LdrpHeapReAlloc(HEAP_ZERO_MEMORY, Destination->String.Buffer, Length);
    }
    else
    {
        NewBuffer = LdrpHeapAlloc(HEAP_ZERO_MEMORY, Length);
...
LdrpAppendAnsiStringToFilenameBuffer(PLDRP_UNICODE_STRING_BUNDLE Destination, ANSI_STRING *Source)
{
    ...
    if (Source->Length)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            TempUnicode.Buffer = Destination->String.Buffer + Destination->String.Length;
