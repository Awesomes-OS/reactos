    ...
    _Out_ PRTLP_PATH_INFO PathInfo)
{
    ...
    /* Get the path type */
    PathInfo->Type = RtlDetermineDosPathNameType_U(FileNameBuffer);
...
NTSTATUS
NTAPI
RtlpWin32NtNameToNtPathName(_In_ RTLP_DosPathNameToRelativeNtPathName_FLAGS Flags,
                                    ...
                                    _Out_opt_ PRTL_RELATIVE_NAME_U RelativeName)
{
    DPRINT("%s(%hhx, \"%wZ\", %p, %p, %p, %p, %p)\n",
        __FUNCTION__,
        Flags, DosName, StaticBuffer, DynamicBuffer, NtName, NtFileNamePart, RelativeName);
    ...
    if (DosName->Length > 8)
    {
        if (RtlPrefixUnicodeString(&RtlpWin32NtRootSlash, DosName, FALSE) || RtlPrefixUnicodeString(&RtlpDosDevicesPrefix, DosName, FALSE))
        {
            ...
            return RtlpWin32NtNameToNtPathName(Flags, DosName, StaticBuffer, DynamicBuffer, NtName, NtFileNamePart, RelativeName);
        }
    }
    ...
    WCHAR LocalStaticBuffer[MAX_PATH];
    UINT_PTR FileNamePartOffset = 0;
    ...
    else
    {
        ...
        FileNamePartOffset = (UINT_PTR)NtFileNamePart - (UINT_PTR)CapturedDosName.Buffer;
    }
    ...
    OffsetsDifference = Prefix.Length - OffsetsDifference;
    ...
    {
        *NtFileNamePart = (PCWSTR)((UINT_PTR)TargetBuffer->Buffer + OffsetsDifference + FileNamePartOffset);
    }
        ...
        if (BufferPathType == RtlPathTypeRelative)
        {
            ...
            if (CurrentDirectory->Handle)
            {
                ...
                if (RtlPrefixUnicodeString(&CurrentDirectory->DosPath, &CapturedDosName, TRUE))
                {
                    ...
                    RelativeName->RelativeName.Buffer = (PWSTR)((UINT_PTR)TargetBuffer->Buffer + Prefix.Length + CurrentDirectory->DosPath.Length);
                    RelativeName->RelativeName.Length = (USHORT)(TargetBuffer->Length - Prefix.Length - CurrentDirectory->DosPath.Length);
...
{
    DPRINT1("CALL TO %s (missing from Windows)", __FUNCTION__);
    __debugbreak();
...
RtlDetermineDosPathNameType_U(IN PCWSTR Path)
{
    ...
    PathString.Length = min(PathString.Length, 10u);
    return RtlDetermineDosPathNameType_Ustr(&PathString);
#endif
}
    ...
    _Out_opt_ PWSTR *ShortName)
{
    ...
    /* Call the extended function, don't care about status (Length will be 0 anyway) */
    RtlGetFullPathName_UEx((PWSTR)FileName,
                           Size,
                           Buffer,
                           ShortName,
                           &Length);
