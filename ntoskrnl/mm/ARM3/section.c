                     ...
                     IN ULONG Flags)
{
    ...
    {
        /* Call Mm API */
        return MiRosUnmapViewOfSection(Process, BaseAddress, PspTestProcessExitingFlag(Process));
    }
                  ...
                  IN PEPROCESS Process)
{
    ...
    {
        const SIZE_T NumberOfBytes = FileName->Length + sizeof(UNICODE_NULL) + sizeof(LDR_DATA_TABLE_ENTRY);
...
#if 1
            LdrEntry->DdagNode = ExAllocatePoolWithTag(NonPagedPool, sizeof(LDR_DDAG_NODE), TAG_MM_LDR_DDAG_NODE);
            if (!LdrEntry->DdagNode)
            {
